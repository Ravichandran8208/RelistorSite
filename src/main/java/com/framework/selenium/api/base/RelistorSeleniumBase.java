...
import com.framework.utils.RelistorReporter;

import io.github.bonigarcia.wdm.WebDriverManager;
import io.github.sukgu.Shadow;

public class RelistorSeleniumBase extends RelistorReporter implements Browser, Element {
	...
	protected void dragAndDropByXandYOffset(WebElement eleSoutce, int eleTargetx,int eleTargety) {
		act = new Actions(getDriver());
		act.dragAndDropBy(eleSoutce, eleTargetx,eleTargety).perform();
	}
	...
	public void click(WebElement ele) {
		...
		try {
			...
			reportStep("The Element " + text + " is Sucessfully clicked " ,"pass");
...
//		 cssValue1 = null;
	...
	public void scrollByXYoffset(int x,int y) {
		// TODO Auto-generated method stub
		try {
			getDriver().executeScript("window.scrollBy("+x+","+y+")");
		} catch (Exception e) {
			reportStep("Scroll to othe Element is not Successfull " + e.getMessage(), "fail", false);
		}
	}
...
//			return link;
	...
	public void verifyAltTag() {
		String alt;
	
//		int count = 1;

		 List<WebElement> altAttribute = getDriver().findElements(By.tagName("img"));
	        
	        Iterator<WebElement> it = altAttribute.iterator();
	        
	        while(it.hasNext()){
	            
	        	alt   = it.next().getAttribute("alt");
	...
	System.out.println("URL is either not configured for anchor tag or it is empty");
	            ...
	            if(url.contains("mailto")){
	            	System.out.println("URL is going to use for Mailing "+url);
	            	reportStep("mailing link="+url, "info");
	            	                continue;
	            }
